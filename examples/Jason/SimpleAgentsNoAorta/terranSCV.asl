...
condition(Building)
	:-	not(friendly(_, Building, _, _, _, _, _)).
...
+gameStart <- !!build("Terran Refinery"); !!build("Terran Supply Depot"); !!build("Terran Barracks"); !!build("Terran Academy"); !!gather.
...
+!build(Building)
	:	Building = "Terran Refinery" &
		...
		distance(MyX,MyY,X,Y,D) &
		.findall([OtherX,OtherY,OtherD], (friendly(Name, "Terran SCV", _, _, _, OtherX, OtherY) & distance(OtherX,OtherY,X,Y,OtherD) & OtherD < D), [])
	<-	!build(Building, X-2, Y-1);!!build(Building).
+!build(Building)
	:	not Building="Terran Refinery" &
		canBuild(Building, X, Y)
	<-	!build(Building, X, Y);!!build(Building).
+!build(Building) <-.wait(200); !!build(Building).
-!build(Building) <-.wait(200); !!build(Building).
...
+!gather
	...
	<-	+busy; gather(Id); .wait(2000); -busy; !!gather.
+!gather 
	:	not(busy) &
		not(gathering(_)) & 
		id(MyId)&
		closest("mineral Field",ClosestId)
	<-	+busy;gather(ClosestId); .wait(2000); -busy; !!gather.
